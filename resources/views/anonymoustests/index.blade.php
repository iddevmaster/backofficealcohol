<x-app-layout>
  <div class="space-y-6 px-4 py-8 max-w-7xl mx-auto">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
      <div>
        <h1 class="text-2xl font-bold text-slate-800">ประวัติการทดสอบ (ไม่ระบุตัวตน)</h1>
        <p class="text-sm text-slate-500">จัดการและตรวจสอบประวัติการตรวจวัดแอลกอฮอล์แบบไม่ระบุตัวตน</p>
      </div>
    </div>

    {{-- Filter Form --}}
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
      <form method="GET" action="{{ route('anonymous-tests.index') }}">
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">ค้นหา</label>
            <input type="text" name="q" value="{{ request('q') }}" placeholder="User ID / SN เครื่อง / ประเภท"
        ...
        <div class="mt-4 flex flex-wrap gap-2">
          ...
          <a href="{{ route('anonymous-tests.index') }}"
        ...
        <table class="min-w-full text-sm">
          <thead class="bg-slate-50">
            <tr>
              ...
              <th class="px-4 py-3 text-left font-semibold text-slate-600">ผู้ใช้งาน (User ID)</th>
              <th class="px-4 py-3 text-left font-semibold text-slate-600">SN เครื่อง</th>
              <th class="px-4 py-3 text-left font-semibold text-slate-600">ประเภทสแกน</th>
          ...
          <tbody class="divide-y divide-slate-100">
            @forelse($test as $index => $row)
              @php
                $isPass = strtolower($row->result) === 'pass';
              @endphp
              <tr class="hover:bg-slate-50 transition-colors">
                ...
                <td class="px-4 py-3">
                  @if($row->image_path)
                    <img src="{{ asset(\Illuminate\Support\Str::startsWith($row->image_path, 'storage/') ? $row->image_path : 'storage/' . $row->image_path) }}" alt="testing image"
                ...
                <td class="px-4 py-3">
                  <div class="font-bold {{ $isPass ? 'text-green-600' : 'text-red-600' }}">
                    {{ number_format($row->value, 2) }} mg/L
                  </div>
                </td>
                ...
                <td class="px-4 py-3 text-slate-600 whitespace-nowrap">
                  {{ $row->scanned_at ? \Carbon\Carbon::parse($row->scanned_at)->format('d/m/Y H:i') : '-' }}
                </td>

                <td class="px-4 py-3 text-center">
                  <div class="flex justify-center gap-2">
                    <a href="{{ route('anonymous-test.receipt', \App\Helpers\HashidsHelper::encode($row->id)) }}"
                    ...
                    <form action="{{ route('anonymous-tests.destroy', \App\Helpers\HashidsHelper::encode($row->id)) }}" method="POST" class="inline">
                      @csrf
                      @method('DELETE')
                      <button type="submit"
