...
<head>
    ...
    <title>ALT Anonymous E-Receipt #{{ str_pad($history->id, 6, '0', STR_PAD_LEFT) }}</title>
...
<body class="bg-slate-100 min-h-screen flex flex-col items-center py-10 px-4">
    ...
    @php
        $isPass = strtolower($history->result) === 'pass';
        $testDate = $history->scanned_at ? \Carbon\Carbon::parse($history->scanned_at) : null;
        $nowDate = \Carbon\Carbon::parse(now('Asia/Bangkok'))->format('d/m/Y  H:i:s');
        $imgSrc = $history->image_path
            ? asset(
                \Illuminate\Support\Str::startsWith($history->image_path, 'storage/')
                    ? $history->image_path
                    : 'storage/' . $history->image_path,
            )
            : null;
    ...
    <div class="mb-6 text-center px-4">
        ...
        <div class="text-base sm:text-xl font-extrabold text-slate-800 tracking-wider text-center">
            ใบรับรองการตรวจวัดแอลกอฮอล์ <br> (Test Receipt - Anonymous)
        </div>
    </div>
            ...
            <div class="receipt-body">
                ...
                <div class="employee-section">
                    <div style="display:flex; align-items:center; gap:14px; flex:1; min-width:0;">
                        <div
                            style="width:48px; height:48px; border-radius:10px; background:linear-gradient(135deg,#64748b,#475569);
                        ...
                        <div style="min-width:0;">
                            <div
                                ...
                                ผู้ใช้งานไม่ระบุตัวตน</div>
                            <div style="font-size:14px; font-weight:700; color:#1e293b; ">
                                User ID: {{ $history->user_id ?? 'ไม่ระบุตัวตน' }}
                            </div>
                        </div>
                    </div>
                ...
                @php
                    $rows = [
                        ...
                        ['label' => 'อุปกรณ์ SN', 'value' => $history->device->serial_num ?? 'N/A', 'mono' => true, 'badge' => true],
                        ['label' => 'ประเภทสแกน', 'value' => $history->scan_type ?? '-', 'mono' => false],
    ...
    <script>
        function downloadReceipt() {
            ...
            html2canvas(el, {
                ...
                link.download =
                    'e_receipt_anon_{{ $history->user_id ?? 'na' }}_{{ $testDate ? $testDate->format('Ymd_His') : 'na' }}.png';
                link.click();
