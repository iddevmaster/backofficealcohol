...
<div class="flex items-center justify-between mb-4">
  ...
  <a href="{{ route('branchesUser.create') }}" class="rounded-md bg-indigo-600 px-4 py-2 text-white hover:bg-indigo-700">+ เพิ่มสาขา</a>
</div>
        ...
        @foreach($branches as $b)
          ...
          <tr>
            ...
            <td class="px-4 py-2 text-right">
              <a href="{{ route('branchesUser.edit', $b) }}" class="inline-flex rounded-md border px-3 py-1.5 hover:bg-gray-50">แก้ไข</a>
              <form action="{{ route('branchesUser.destroy', $b) }}" method="post" class="inline" onsubmit="return confirm('ลบรายการนี้หรือไม่?');">
                @csrf @method('DELETE')
