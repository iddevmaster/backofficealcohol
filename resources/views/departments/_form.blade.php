@csrf
<div class="mb-4">
  <label class="block text-sm font-medium mb-1">รหัสฝ่าย (dpm_id)</label>
  <input type="text" name="dpm_id"
         class="w-full rounded-md border-gray-300 bg-gray-100 focus:border-indigo-500 focus:ring-indigo-500 @error('dpm_id') border-red-500 @enderror"
         value="{{ old('dpm_id', $department->dpm_id) }}"
         placeholder="{{ !$department->exists ? 'ระบบจะสร้างให้อัตโนมัติ' : '' }}"
         {{ !$department->exists ? 'disabled' : 'readonly' }}>
  @error('dpm_id') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
</div>

<div class="mb-4">
  <label class="block text-sm font-medium mb-1">ชื่อฝ่าย (name)</label>
