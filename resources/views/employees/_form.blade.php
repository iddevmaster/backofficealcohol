@php
  ...
  $oldPre = old('prefix_id', $employee->prefix_id ?? '');
...
<div x-data="{
  ...
  <div>
    ...
    <select name="prefix_id" x-model="prefix" class="w-full rounded border-gray-300">
  ...
  <div>
    ...
    <input name="phone" class="w-full rounded border-gray-300" value="{{ old('phone', $employee->phone ?? '') }}">
  ...
  <div>
    ...
    <select name="org_id" x-model="org" @change="resetBrn()" class="w-full rounded border-gray-300" required>
      <option value="">-- เลือก --</option>
  ...
  <div>
    ...
    <select name="brn_id" x-model="brn" @change="resetDpm()" class="w-full rounded border-gray-300" :disabled="!org">
      <option value="">-- เลือก --</option>
      <template x-for="b in brnList()" :key="b.id">
        <option :value="String(b.id)" x-text="`${b.name} (${b.id})`"></option>
      </template>
    </select>
  ...
  <div>
    ...
    <select name="dpm_id" x-model="dpm" class="w-full rounded border-gray-300" :disabled="!brn">
      ...
      <template x-for="d in dpmList()" :key="d.id">
        <option :value="String(d.id)" x-text="`${d.name} (${d.id})`"></option>
      </template>
    </select>
  ...
  <div class="flex items-center gap-6 md:col-span-3">
    <label class="inline-flex items-center gap-2">
      ...
      <input type="checkbox" name="fingerprint_registered" value="1" @checked(old('fingerprint_registered', $employee->fingerprint_registered ?? false))>
      <span>ลงทะเบียนลายนิ้วมือแล้ว</span>
    </label>

    <label class="inline-flex items-center gap-2">
      <input type="hidden" name="status" value="0">
      <input type="checkbox" name="status" value="1" @checked(old('status', $employee->status ?? true))>
