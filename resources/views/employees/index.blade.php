<x-app-layout>
  ...
  <form method="get" class="mb-4">
    <div class="flex gap-2">
      ...
      <a href="{{ route('employees.create') }}" class="ml-auto rounded bg-blue-600 text-white px-3 py-2">+
        เพิ่มพนักงาน</a>
    </div>
  </form>
        ...
        <tbody class="divide-y">
          @foreach($employees as $e)
            <tr>
              ...
              <td class="px-4 py-2">{{ $e->Branches->name ?? '-' }}</td>
              ...
              <td class="px-4 py-2 text-right">
                ...
                <form action="{{ route('employees.destroy', $e) }}" method="post" class="inline"
                  onsubmit="return confirm('ลบพนักงาน?')">
                  @csrf @method('DELETE')
