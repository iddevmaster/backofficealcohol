...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
  <!-- <div>
    ...
    <input type="text" name="org_id" value="{{ old('org_id', $organization->org_id ?? '') }}"
  ...
  <div>
    ...
    <input type="text" name="name" value="{{ old('name', $organization->name ?? '') }}"
      placeholder="เช่น บริษัท ไอดีไดรฟ์ จำกัด"
  ...
  <div>
    <label class="block text-sm font-medium mb-1">ตัวย่อองค์กร</label>
    <input type="text" name="org_code" value="{{ old('org_code', $organization->org_code ?? '') }}"
      placeholder="ตัวย่อองค์กร เช่น IDD"
      class="w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 @error('org_code') border-red-500 @enderror">
    @error('org_code') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
  </div>

  <div class="md:col-span-2">
    <label class="block text-sm font-medium mb-1">โลโก้ (URL/Path)</label>
    <input type="text" name="logo" value="{{ old('logo', $organization->logo ?? '') }}"
  ...
  <div class="md:col-span-2 flex items-center gap-3">
    ...
    <input type="checkbox" name="status" value="1"
      class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" {{ old('status', $organization->status ?? false) ? 'checked' : '' }}>
