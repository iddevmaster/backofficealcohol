...
<div class="flex items-center justify-between mb-4">
  ...
  <a href="{{ route('usersUser.create') }}" class="rounded-md bg-blue-600 px-4 py-2 text-white hover:bg-blue-700">+ เพิ่มผู้ใช้</a>
</div>

<form method="get" class="mb-4">
  <div class="flex gap-2">
    <input type="text" name="q" value="{{ $q }}" placeholder="ค้นหา username / ชื่อ"
        ...
        @foreach($users as $u)
          <tr>
            ...
            <td class="px-4 py-2 text-right">
              <a href="{{ route('usersUser.show', $u) }}" class="inline-flex rounded-md border px-3 py-1.5 hover:bg-gray-50">ดู</a>
              <a href="{{ route('usersUser.edit', $u) }}" class="inline-flex rounded-md border px-3 py-1.5 hover:bg-gray-50">แก้ไข</a>
              <form action="{{ route('usersUser.destroy', $u) }}" method="post" class="inline" onsubmit="return confirm('ลบรายการนี้หรือไม่?');">
                @csrf @method('DELETE')
